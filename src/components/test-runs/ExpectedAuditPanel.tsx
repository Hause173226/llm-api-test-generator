...
export default function ExpectedAuditPanel({
  ...
  return (
    <section className={cn("rounded-lg border border-outline-variant/20 bg-surface-container-lowest dark:bg-slate-900 p-3", className)}>
      ...
      {hasUnverified && (
        <div className="mt-2 flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-2 text-[11px] text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100">
          ...
          <span>Some expected checks are AI inferred, unknown, or missing backend evidence. Do not treat them as authoritative until reviewed.</span>
        </div>
      )}
                ...
                .map((item, index) => {
                  ...
                  return (
                    <details
                      ...
                      className="rounded-md border border-outline-variant/20 bg-white p-2 text-xs dark:bg-slate-950/60"
                    >
                      ...
                      <div className="mt-2 space-y-1 border-t border-outline-variant/10 pt-2 text-[11px] text-on-surface-variant">
                        ...
                        <div>Evidence: {evidence || "No source provided by backend"}</div>
                        {expectationSource && (
                          <div>Legacy expectation source: {expectationSource}</div>
                        )}
                      </div>
                    </details>
                  );
